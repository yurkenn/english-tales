...
// --- MOCK DATA (Copied from data/mock.ts for standalone execution) ---
...
const mockStories = [
    ...
    },
    {
        ...
        isFeatured: true, // Featured story in mock
    },
    {
        id: '3',
        title: '1984',
...
// Combine hardcoded genres and tags from stories
const allCategories = new Set(genres.filter(g => g !== 'For You')); // 'For You' is a UI tab, not a content category usually
mockStories.forEach(s => s.tags.forEach(t => allCategories.add(t)));
...
console.log(`Generated ${allCategories.size} categories`);

// 2. Process Authors
const authorMap = new Map(); // Name -> ID
const authors = new Set();
// Add featured author first to give better data if he exists in stories
// But for now, just iterating stories
mockStories.forEach(s => authors.add(s.author));

authors.forEach(name => {
    const id = `author-${name.toLowerCase().replace(/[^a-z0-9]/g, '-')}`;
    authorMap.set(name, id);

    // Check if we have extra data for this author (like Neil Gaiman)
    addDoc({
        _id: id,
        _type: 'author',
        name: name,
        slug: { _type: 'slug', current: id.replace('author-', '') },
        // Placeholder bio
        bio: `${name} is a celebrated author known for their compelling storytelling.`,
    });
});

console.log(`Generated ${authors.size} authors`);

// 3. Process Stories
mockStories.forEach(story => {
    const authorId = authorMap.get(story.author);
    const categoryRefs = story.tags
        .map(tag => categoryMap.get(tag))
        .filter(id => id) // remove undefined
        .map(id => ({ _type: 'reference', _ref: id, _key: id })); // Sanity arrays need keys

    // Transform content string to Portable Text blocks
    const contentBlocks = [
        {
            _type: 'block',
            _key: 'b1',
    ...
    addDoc({
        ...
        _type: 'story',
        title: story.title,
        slug: { _type: 'slug', current: story.title.toLowerCase().replace(/[^a-z0-9]/g, '-') },
        ...
        publishedAt: new Date(story.createdAt).toISOString(),
        // Handling Image
        // If it's a URL, Sanity CLI can import it if we use special _sanityAsset notation
        // E.g. image@https://...
        coverImage: {
            _type: 'image',
            _sanityAsset: `image@${story.coverImage}`
        }
    });
});

console.log(`Generated ${mockStories.length} stories`);
...
mockReviews.forEach(review => {
    addDoc({
        ...
        story: { _type: 'reference', _ref: `story-${review.storyId}` },
...
console.log(`Generated ${mockReviews.length} reviews`);

// WRITE TO FILE
const outputPath = path.join(__dirname, 'seed.ndjson');
fs.writeFileSync(outputPath, ndjsonLines.join('\n'));
console.log(`\nSuccessfully wrote seed data to ${outputPath}`);
